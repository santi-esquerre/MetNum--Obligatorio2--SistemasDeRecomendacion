## -*- texinfo -*-
## @node tychonoffRegularization
## @deftypefn {Function} {@var{P, Q, errAjuste, errTest} =} tychonoffRegularization (path, maxiter, f, lambda, saveError)
## PRE: Existen los archivos de datos puntajes_ajuste.csv y puntajes_test.csv.
## POST: Se minimiza el error cuadrático medio de los puntajes calculados a través del método de minimización alternada con regularización de Tychonoff, se hacen maxiter iteraciones, se usan f factores latentes y se aplica un factor de regularización lambda, saveError es un booleano que indica si se guardan los errores en cada iteración.
## @end deftypefn
